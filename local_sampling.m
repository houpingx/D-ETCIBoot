function [local_truth,estvariance,t] = local_sampling( dataset,iteration,alpha,K,truth)
t1 = tic;
...
while(ii<= iteration)
    ...
    for i=1:noe % for each entity
        ...
        local_truth( list_entry(i),2 ) = mean( jack_truth );
...
%         end
    ...
    truth_matrix=local_truth((entry_ic),:);
	% ----------------------------------------------------------------------
end
% put truth with entity index
%local_truth=[list_entry,local_truth];
t = toc(t1);
end
